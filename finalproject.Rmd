...
clean_schools %>%
  ...
  theme(
        ...
        text=element_text(size=12, 
                          family="serif"))+
  scale_x_continuous(breaks = round(seq(min(clean_schools$year), max(clean_schools$year), by = 2),1)) +
  labs(title = "Average Support Services Expenditure over time for the different regions", 
       y = "Average Support Services Expenditure (in millions $)", 
       x = "Year",
       color = "Region")
```
